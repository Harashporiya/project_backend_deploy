...
function MoviesShow() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${DEPLOY_URL}/api/find`);
                setDataMovies(response.data.getMovies);
    ...
    return (
        <div className="bg-gray-900 min-h-screen text-white p-4">
            ...
            <div className="flex flex-wrap pt-2 justify-center ">
               ...
               {dataMovies && dataMovies.map((datas1) => (
                    <div id='card' className='rounded-lg shadow-2xl bg-white m-4 ' key={datas1.id}>
                        <img id='img' className='cover rounded-t-lg h-[400px] w-[400px]' src={datas1.Image} alt='' />
                        <div className='p-4 text-black'>
